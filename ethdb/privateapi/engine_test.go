...
func TestMockDownloadRequest(t *testing.T) {
	...
	events := NewEvents()
	backend := NewEthBackendServer(ctx, nil, db, events, nil, &params.ChainConfig{TerminalTotalDifficulty: common.Big1}, beaconRequestList, statusCh, nil, false, false)
...
func TestNoTTD(t *testing.T) {
	...
	events := NewEvents()
	backend := NewEthBackendServer(ctx, nil, db, events, nil, &params.ChainConfig{}, beaconRequestList, statusCh, nil, false, false)
